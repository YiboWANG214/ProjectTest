...
def run_tests_and_collect_errors():
    ...
    try:
        results1 = subprocess.run(["npm", "install", "jest", "--save-dev", "jest-environment-jsdom", "@babel/core", "@babel/preset-env", "babel-jest", "chai@3"], text=True, capture_output=True)
        results = subprocess.run(["npm", "test", "--", "--coverage"], text=True, capture_output=True, encoding="utf-8")
        # filtered_output = "\n".join(line.strip() for line in results.stdout.splitlines() if line)
...
def generate_tests(
    ...
    tests_output = json.load(open(f"./generated_tests/{gen_model}_Data/ProjectTestjs_output/original_output_{max_tokens}.json"))
            ...
            try:
                ...
                for file_name in os.listdir(generated_test_folder):
                    if file_name.startswith(prompt["classname"]):
                        ...
                        destination_file = os.path.join(os.path.join(output_folder_initial, prompt["classname"]), file_name)
